...
def generate_fake_data(num_samples=1000):
    ...
    for _ in range(num_samples):
        ...
        score = 0
        if u_brand == s_specialty: score += 0.4 # 品牌匹配加分
        if distance < 10: score += 0.3          # 距离近加分
        if s_rating > 4.5: score += 0.2         # 评分高加分
        if price < 0.5: score += 0.1            # 便宜加分
    ...
    def _load(self):
        ...
        if os.path.exists(self.file_path):
            try:
                ...
                print(f"[{self.feature_name}] 已加载词表，当前大小: {len(self.token2id)}")
...
class FeatureProcessor:
    def __init__(self, config):
       ...
       # 初始化每个稀疏特征的 Mapper
        for feat, limit in config['vocab_limits'].items():
            self.mappers[feat] = PersistentVocabMapper(feature_name=feat,vocab_dir=config['vocab_dir'],max_size=limit)
    ...
    def transform(self, df):
        ...
        for feat in self.config['dense_features']:
            ...
            # 简单的 MaxMin 归一化模拟
            if feat == 'distance_km': vals = vals / 50.0 
            if feat == 'rating': vals = vals / 5.0
            dense_list.append(vals.reshape(-1, 1))
        dense_tensor = torch.tensor(np.concatenate(dense_list, axis=1), dtype=torch.float32)
...
class DeepFM(nn.Module):
    def __init__(self, config):
        ...
        deep_input_dim = (self.num_sparse * config['embed_dim']) + self.num_dense + self.text_dim
    ...
    def forward(self, sparse_inputs, dense_inputs, text_inputs):
        ...
        # 4. 计算 FM Logit
        # 4.1 FM 一阶
        fm_1st_s = sum([self.fm_1st_sparse[f](sparse_inputs[f]) for f in self.config['sparse_features']])
...
def train_pipeline():
    ...
    for epoch in range(CONFIG['epochs']):
        ...
        for batch_idx, (s, d, t, label) in enumerate(dataloader):
            ...
            d, t, label = d.to(CONFIG['device']), t.to(CONFIG['device']), label.to(CONFIG['device'])
        ...
        print(f"    Epoch {epoch+1}/{CONFIG['epochs']}, Avg Loss: {total_loss/len(dataloader):.4f}")
...
def predict_pipeline(model, processor):
    ...
    for i, req in enumerate(new_requests):
        ...
        print(f"客户: {req['brand_name']} (ID映射为: {brand_id}) | 距离: {req['distance_km']}km")
        print(f"-> 推荐得分: {probs[i].item():.4f}")
        if req['brand_name'] == '保时捷':
            print("   (注: 保时捷是新词，分配了新ID，得分主要靠BGE文本向量和数值特征支撑)")
        print("-" * 30)
